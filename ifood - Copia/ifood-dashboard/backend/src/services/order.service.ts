...
const create = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Erro ao importar pedidos. Verifique os dados ou tente novamente mais tarde.', error });
  } finally {
    client.release();
  }
};
...
const getAll = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const pedidosComItens = await Promise.all(
      pedidos.rows.map(async (pedido: PedidoDB) => {
        const itens = await pool.query(
          `SELECT * FROM pedido_items WHERE pedido_id = $1`,
          [pedido.id]
        );
        return { ...pedido, items: itens.rows as PedidoItem[] };
      })
    );
    ...
    res.status(500).json({ message: 'Erro ao buscar pedidos. Tente novamente mais tarde.', error });
  }
};

// Exportando as funções do controlador de pedidos
